...
const zig_version: enum { release, master } = if (std.mem.eql(u8, @import("builtin").zig_version_string, "0.11.0")) .release else .master;
...
pub fn create(b: *std.Build, opt: struct {
    ...
    result.* = GitRepoStep{
        ...
        .branch = opt.branch,
        .sha = opt.sha,
        .path = if (opt.path) |p| b.allocator.dupe(u8, p) catch @panic("OOM") else b.pathJoin(&.{ "dep", name }),
...
fn make(step: *std.Build.Step, prog_node: *std.Progress.Node) !void {
    ...
    const p = step.owner.build_root.path.?;
    var dir = try std.fs.openDirAbsolute(p, .{});
    defer dir.close();
    dir.access(self.path, .{}) catch {
...
fn run(builder: *std.Build, argv: []const []const u8) !void {
    ...
    child.env_map = switch (zig_version) {
        .master => &builder.graph.env_map,
        .release => builder.env_map,
    };
